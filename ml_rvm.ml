...
let rec get_int n =
  ...
  let y = n * 46 in
  if x < 46
  then (print_int (y + x); print_newline (); x + y)
  else get_int (y + (x - 46))
...
let show_decode_step (op, n, x, d) =
  Printf.printf "pos : %d | x : %d | op : %d | n : %d | d : %d | symtbl : %d | stack : %d\n"
   (!pos) x op n d (!symtbl) (!stack)

let decode () =
  let _ =
    build_symtbl ();
    print_endline "symbol table built"
  in
...
let show_step name =
  ...
  print_string "step "; print_int !step;
  print_string " : "; print_string name;
  print_string " | stack : "; print_int !stack;
...
let rec run () =
  ...
  Int 0 ->
    ...
    | Triplet i -> (* calling a lambda *)
      ...
      if is_rib next
      then print_debug "jump"
      else print_debug "call";
